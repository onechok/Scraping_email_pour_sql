...
def connect_to_db():
    try:
        ...
        print("Erreur de connexion à la base de données MariaDB :", e)
        connection = None
    return connection
    ...
    if connection:
        try:
            with connection.cursor() as cursor:
                cursor.execute("CREATE TABLE IF NOT EXISTS emails (id INT NOT NULL AUTO_INCREMENT, nom VARCHAR(255) DEFAULT '', mail VARCHAR(255) NOT NULL, url VARCHAR(255) NOT NULL, PRIMARY KEY (id)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;")
                connection.commit()
                print("Table 'emails' créée avec succès dans la base de données 'scrape'.")
        except Exception as e:
            print("Erreur de création de la table 'emails' :", e)
        ...
        for mail in mails:
            try:
                with connection.cursor() as cursor:
                    ...
                    connection.commit()
                print("Adresse e-mail enregistrée :", mail)
            except Exception as e:
                print("Erreur d'enregistrement de l'adresse e-mail :", e)
    else:
        try:
            with connection.cursor() as cursor:
                sql = "INSERT INTO emails (nom, mail, url) VALUES (%s, %s, %s)"
                cursor.execute(sql, (titre, "", url))
                connection.commit()
            print("Aucune adresse e-mail trouvée. Enregistrement du titre et de l'URL.")
        except Exception as e:
            print("Erreur d'enregistrement du titre et de l'URL :", e)
    ...
    try:
        with connection.cursor() as cursor:
            ...
            if results:
                ...
                print("Aucune information trouvée dans la base de données pour l'URL :", url)
    except Exception as e:
        print("Erreur de récupération des informations depuis la base de données :", e)
...
def close_db_connection(connection):
    if connection:
        try:
            ...
            print("Erreur de fermeture de la connexion à la base de données MariaDB :", e)
